...
function getPrefix(threadID) {
	...
	let prefix = global.VexaBot.config.prefix;
	const threadData = global.db.allThreadData.find(t => t.threadID == threadID);
	if (threadData) {
    if (threadData.data.prefix == "NO") return "";
...
function message(api, event) {
    ...
    stream: async (T, U) => {
      let F;
      if (!U) {
        F = { attachment: await utils.getStreamFromUrl(T) };
      } else {
        F = { body: T, attachment: await utils.getStreamFromUrl(U) };
      }
      api.sendMessage(F, event.threadID, event.messageID);
    },
    react: async (ic) => {
      	return await api.setMessageReaction(ic, event.messageID, () => { }, true);
    },
    edit: async (text, MID) => {
      let m;
      if (MID) m = MID;
      if (event?.messageReply?.messageID) m = event?.messageReply?.messageID;
      if (!m) return;
      return await api.editMessage(text, m);
    },
		...
		er: async (err) => await sendMessageError(err),
...
async function IMGBB(file) {
  ...
  try {
    ...
    if (regCheckURLL.test(file) === true) {
      type = "url";
    }
    if (
