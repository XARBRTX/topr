...
module.exports = {
  config: {
    ...
    category: "box chat",
    guide: {
ar: "   {pn} [@tag|uid|link fb|reply] [<سبب الحظر>|اتركه فارغاً إذا لم يكن هناك سبب]: حظر العضو من المجموعة"
        + "\n   {pn} check: التحقق من الأعضاء المحظورين وطرد تلك الأعضاء من المجموعة"
        + "\n   {pn} unban [@tag|uid|link fb|reply]: إلغاء حظر العضو من المجموعة"
...
en: "   {pn} [@tag|uid|fb link|reply] [<reason>|leave blank if no reason]: Ban user from box chat"
  ...
  langs: {
    ar: {
      notFoundTarget: "⚠️ | يرجى وضع علامة على الشخص الذي ترغب في حظره أو إدخال معرف الحساب أو رابط الفيسبوك أو الرد على رسالة الشخص الذي ترغب في حظره",
      notFoundTargetUnban: "⚠️ | يرجى وضع علامة على الشخص الذي ترغب في إلغاء حظره أو إدخال معرف الحساب أو رابط الفيسبوك أو الرد على رسالة الشخص الذي ترغب في إلغاء حظره",
      userNotBanned: "⚠️ | المستخدم الذي يحمل معرف %1 لم يتم حظره من هذه المجموعة",
      unbannedSuccess: "✅ | تم إلغاء حظر %1 من المجموعة بنجاح!",
      cantSelfBan: "⚠️ | لا يمكنك حظر نفسك!",
      cantBanAdmin: "❌ | لا يمكنك حظر المشرفين!",
      existedBan: "❌ | هذا الشخص محظور بالفعل!",
      noReason: "لا يوجد سبب",
      bannedSuccess: "✅ | تم حظر %1 من المجموعة بنجاح!",
      needAdmin: "⚠️ | يحتاج البوت إلى صلاحيات المشرف لطرد الأعضاء المحظورين",
      noName: "مستخدم فيسبوك",
      noData: "📑 | لا يوجد أي أعضاء محظورين في هذه المجموعة",
      listBanned: "📑 | قائمة الأعضاء المحظورين في هذه المجموعة (الصفحة %1/%2)",
      content: "%1/ %2 (%3)\nالسبب: %4\nوقت الحظر: %5\n\n",
      needAdminToKick: "⚠️ | العضو %1 (%2) تم حظره من المجموعة، لكن البوت ليس لديه صلاحيات المشرف لطرد هذا العضو، يرجى منح البوت صلاحيات المشرف لطرد هذا العضو",
      bannedKick: "⚠️ | %1 تم حظره من المجموعة مسبقًا!\nمعرف الحساب: %2\nالسبب: %3\nوقت الحظر: %4\n\nتم طرد هذا العضو تلقائيًا من البوت"
    },
en: {
  ...
  atCall: async function ({ message, event, args, threadsData, getLang, usersData, api }) {
    ...
    const time = moment().tz(global.VexaBot.config.timeZone).format('HH:mm:ss DD/MM/YYYY');
    const data = {
      id: target,
      time,
      ...
      if (members.some(item => item.userID == target)) {
        if (adminIDs.includes(api.getCurrentUserID()))
          ...
          message.send(getLang('needAdmin'), (err, info) => {
            global.VexaBot.Event.push({
              messageID: info.messageID,
              run: ({ event }) => {
                if (event.logMessageType === "log:thread-admins" && event.logMessageData.ADMIN_EVENT == "add_admin") {
                  const { TARGET_ID } = event.logMessageData;
                  if (TARGET_ID == api.getCurrentUserID()) {
                    api.removeUserFromGroup(target, event.threadID, () => global.VexaBot.Event = global.VexaBot.Event.filter(item => item.messageID != info.messageID));
                  }
                }
              }
            });
                ...
                global.VexaBot.Event.push({
                  ...
                  run: ({ event }) => {
                    if (event.logMessageType === "log:thread-admins" && event.logMessageData.ADMIN_EVENT == "add_admin") {
                      const { TARGET_ID } = event.logMessageData;
                      if (TARGET_ID == api.getCurrentUserID()) {
                        api.removeUserFromGroup(userFbId, event.threadID, () => global.VexaBot.Event = global.VexaBot.Event.filter(item => item.messageID != info.messageID));
                      }
                    }
                  }
                });
