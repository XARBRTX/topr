...
module.exports = {
  config: {
    ...
    category: "game",
    guide: {
      ar: "  {pn} [4 | 5 | 6] [single | multi]: تلعب سولو ولا جماعي."
        + "\n    4 5 6 هي عدد أرقام الرقم و ينصح ب4 للمبتدأين"
        + "\n    سولو | جماعي نمط الفريق اذا بدك تلعب وحد حط سولو (single)."
        + "\n   مثلا أكتب:"
        + "\n    {pn}"
        + "\n    {pn} 4 single"
        + "\n"
        + "\n   طريقة اللعب: "
        + "\n\t\tيقوم البوت بإرسال صورة اللعبة و ترد برقم عشوائي و يرسل لك صورة اخرى فيها رقمك العشوائي في الوسط و رقمين في واحد في اليمين و الاخر في اليسار."
        + "\n الرقم الذي في اليمين يعني عدد الأرقام الصحيحة و الرقم الذي في اليسار يعني عدد الأرقام التي في الموضع الصحيح مقارنة برقم الجواب."
        + "\n الرقم الصحيح مكون من أرقام مختلفة كليا لا يمكن أن تجد رقمين متشابهين فيه و يمكن أن يحتوي على رقم 0 أيضا.",
  ...
  langs: {
    ar: {
      charts: "🏆 | الرانك:\n%1",
      pageInfo: "الصفحة %1/%2",
      noScore: "⭕ | لم يحصل أحد على رانك.",
      noPermissionReset: "⚠️ | لا تملك صلاحية لتصفر الرانك.",
      notFoundUser: "⚠️ | لا يمكن إيجاد المعلومات عن صاحب المعرف %1..",
      userRankInfo: "🏆 | معلومات الرانك:\nالإسم: %1\nالسكور: %2\nعدد مرات اللعب: %3\nعدد مرات الفوز: %4\n%5\nعدد مرات الخسارة: %6\nنسبة الفوز: %7%\nزمن إجمالي اللعب: %8",
      digits: "%1 أرقام: %2",
      resetRankSuccess: "✅ | تم حذف سكور الكل.\n\tإفشخوه 🌝💔🙎‍♀️",
      invalidCol: "⚠️ | أدخل عدد الأرقام حسب عدد الخانات.",
      invalidMode: "⚠️ | سولو أو سكواد 🙎‍♀️؟",
      created: "✅ | تم إنشاء اللعبة بنجاح.",
      gameName: "GUESSNUM-VEXA",
      gameGuide: "♪ | Producted by allou",
      gameNote: "♪ | Game run in Vexa",
      replyToPlayGame: "🎮 | رد على الصورة بالأرقام التي حزرتها.",
      invalidNumbers: "⚠️ | أدخل %1 أرقام فقط.",
      win: "🎉✅ %1 [%2] [%3]",
      loss: "🙎‍♀️ | الرقم الصحيح هو %1."
    },
    en: {
      charts: "🏆 | Ranking:\n%1",
      pageInfo: "Page %1/%2",
  ...
  atCall: async function ({ message, event, getLang, commandName, args, globalData, usersData, role }) {
    ...
    else if (args[0] == "reset" || args[0] == "تصفير") {
      if (role < 2)
    ...
    const mode = args.join(" ").match(/(single|multi|-s|-m|solo|squad|فردي|جماعي|سولو|سكواد)/)?.[1] || "single";
...
console.log(options.answer);
  ...
  atReply: async ({ message, Reply, event, getLang, globalData }) => {
    const { gameData: oldGameData, commandName } = Reply;
    ...
    global.VexaBot.atReply.delete(Reply.messageID);
